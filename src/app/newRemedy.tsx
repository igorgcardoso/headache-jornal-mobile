...
export default function NewRemedy() {
  ...
  const { mutate, isLoading } = useMutation(
    async ({ name }: RemedyData) => {
      const response = await api.post(QUERY_URLS.remedies.create(), {
        name,
      });
    ...
    },
    {
      onSuccess: () => {
        Toast.show({
          type: 'success',
          text1: 'Success',
          text2: 'Remedy created successfully',
        });
        queryClient.invalidateQueries(QUERY_KEYS.remedies.list());
        setValue('name', '');
      },
